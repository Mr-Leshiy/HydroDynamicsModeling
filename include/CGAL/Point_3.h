...
class Point_3 : public R_::Kernel_base::Point_3 {
 ...
 public:
  ...
  Point_3(const Rep& p) : Rep(p) {}

  explicit Point_3(const Weighted_point_3& wp) : Rep(wp.point()) {}
  ...
  template <typename T1, typename T2, typename T3>
  Point_3(const T1& x, const T2& y, const T3& z, HParticle* el)
      : store_el(el),
        Rep(typename R::Construct_point_3()(Return_base_tag(), x, y, z)) {}

  Point_3(const RT& hx, const RT& hy, const RT& hz, const RT& hw)
      : Rep(typename R::Construct_point_3()(
            Return_base_tag(), hx, hy, hz, hw)) {}

  typename cpp11::result_of<typename R::Compute_x_3(Point_3)>::type x() const {
    return typename R::Compute_x_3()(*this);
  }

  typename cpp11::result_of<typename R::Compute_y_3(Point_3)>::type y() const {
    return typename R::Compute_y_3()(*this);
  }

  typename cpp11::result_of<typename R::Compute_z_3(Point_3)>::type z() const {
    return typename R::Compute_z_3()(*this);
  }

  typename cpp11::result_of<typename R::Compute_hx_3(Point_3)>::type hx()
      const {
    return R().compute_hx_3_object()(*this);
  }

  typename cpp11::result_of<typename R::Compute_hy_3(Point_3)>::type hy()
      const {
    return R().compute_hy_3_object()(*this);
  }

  typename cpp11::result_of<typename R::Compute_hz_3(Point_3)>::type hz()
      const {
    return R().compute_hz_3_object()(*this);
  }

  typename cpp11::result_of<typename R::Compute_hw_3(Point_3)>::type hw()
      const {
    return R().compute_hw_3_object()(*this);
  }

  typename cpp11::result_of<typename R::Compute_x_3(Point_3)>::type cartesian(
      int i) const {
  ...
  typename cpp11::result_of<typename R::Compute_x_3(Point_3)>::type operator[](
      int i) const {
    return cartesian(i);
  }
  ...
  Bbox_3 bbox() const { return R().construct_bbox_3_object()(*this); }

  Point_3 transform(const Aff_transformation_3& t) const {
    return t.transform(*this);
  }

  Point_3<R_>& operator+=(const Vector_3& v) {
    *this = R().construct_translated_point_3_object()(*this, v);
    return *this;
  }

  Point_3<R_>& operator-=(const Vector_3& v) {
    *this = R().construct_translated_point_3_object()(
        *this, R().construct_opposite_vector_3_object()(v));
    return *this;
  }
};

template <class R>
inline bool operator==(const Origin& o, const Point_3<R>& p) {
  return p == o;
}
...
std::ostream& insert(std::ostream& os,
  ...
  switch (get_mode(os)) {
    ...
    case IO::BINARY:
      ...
      return os;
    default:
      return os << "PointH3(" << p.hx() << ", " << p.hy() << ", " << p.hz()
                << ", " << p.hw() << ')';
  }
}
...
template <class R>
std::istream& extract(std::istream& is, Point_3<R>& p, const Cartesian_tag&) {
  ...
  if (is) p = Point_3<R>(x, y, z);
  return is;
}

template <class R>
std::istream& extract(std::istream& is, Point_3<R>& p, const Homogeneous_tag&) {
